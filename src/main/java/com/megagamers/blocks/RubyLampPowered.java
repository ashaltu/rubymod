...
public class RubyLampPowered extends Block{
	private final boolean isOn;
	public RubyLampPowered(boolean isOn) {
	...
	public void onBlockAdded(World world, int x,int y, int z){
		if(!world.isRemote){
			if(this.isOn && !world.isBlockIndirectlyGettingPowered(x, y, z)){
				...
				world.setBlock(x,y,z,MBlocks.rubyLampOnPowered,0,2);
			}
		}
	}
	...
	if(!world.isRemote && this.isOn && !world.isBlockIndirectlyGettingPowered(x, y, z)){
		world.setBlock(x, y, z, MBlocks.rubyLampOffPowered,0,2);
		}
	}
	public Item getItemDropped(int i,Random random,int j){
		return Item.getItemFromBlock(MBlocks.rubyLampOffPowered);
	}
	protected ItemStack createStackedBlock(int i){
		return new ItemStack(MBlocks.rubyLampOffPowered);
	}
}
